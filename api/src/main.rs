...
use clap::Parser;
use metta_kg::{db, rocket, cli::Cli};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mork_server_url = cli.mork_server_url.unwrap_or_else(|| "http://127.0.0.1:8001".to_string());
    let mettakg_api_url = cli.mettakg_api_url.unwrap_or_else(|| "http://127.0.0.1:8000".to_string());

    let final_cli = Cli {
        database_url: Some(database_url.clone()),
        mettakg_frontend_url: cli.mettakg_frontend_url.or_else(|| Some("http://127.0.0.1:3000".to_string())),
