...
const RestrictionPage: Component = () => {
  ...
  const canSubmit = () => {
    ...
    const patternsOk = state.patterns.every((p: Item) => (p.value || "").trim().length > 0);
    const templateOk = state.templates.some((t: Item) => (t.value || "").trim().length > 0);
  ...
  return (
    <div class="ml-10 mt-8">
      <CommandCard title="Restriction" description="Filter path entries by prefix set into target namespace">
          ...
          <div class="lg:col-span-1">
            <Card class="sticky top-4">
              <CardHeader>
                ...
                <CardDescription>Conceptual transform for restriction</CardDescription>
              </CardHeader>
              <CardContent>
                <pre class="text-sm font-mono bg-muted p-3 rounded overflow-auto">{buildTransformPreview()}</pre>
                <Button variant="default" size="sm" onClick={copyExpression} class="w-full mt-4">
                ...
                >
                  ...
                  <Show when={isLoading()} fallback={<Show when={isPolling()} fallback={"Run Restriction"}>Waiting for results...</Show>}>
                    Processing...
                  </Show>
                </Button>
              </CardContent>
